...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 3. Compute performance modifier
    ...
    if (isWin) {
      ...
      // Hint penalty: 5% per hint, cap 30%
      const hintPenalty = Math.max(0.7, 1 - hintsUsed * 0.05);

      // No undo penalty — undos only affect move count
    ...
    const newStreak = isWin ? (profile.current_streak as number) + 1 : 0;
    await supabaseAdmin.from('profiles').update({
      rating: newRating,
      games_played: gamesPlayed + 1,
      games_won: (profile.games_won as number) + (isWin ? 1 : 0),
      current_streak: newStreak,
      best_streak: Math.max(profile.best_streak as number, newStreak),
